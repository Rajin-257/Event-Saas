...
exports.postRegister = asyncHandler(async (req, res) => {
  ...
  const verificationURL = `${config.app.url}/auth/verify-email/${user.id}/${verificationToken}?expires=${Date.now() + 24 * 60 * 60 * 1000}`;
  ...
  try {
    ...
    req.flash('error_msg', 'Failed to send verification email. Please try registering again.');
...
exports.postForgotPassword = asyncHandler(async (req, res) => {
  ...
  // Check if user is verified
  if (!user.verify_email) {
    req.flash('error_msg', 'Please verify your email before resetting password');
    return res.redirect('/auth/forgot-password');
  }
  ...
  try {
    ...
    req.flash('error_msg', 'Failed to send password reset OTP. Please try again.');
